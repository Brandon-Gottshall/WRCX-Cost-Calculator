  ...
  const data = [
    {
      ...
      breakdown: [
        { name: "Live Ads", value: revenue.liveAdRevenue },
        { name: "Paid Programming", value: revenue.paidProgrammingRevenue },
        { name: "VOD Ads", value: revenue.vodAdRevenue },
      ],
    },
    {
      name: "Cost",
      value: totalCost,
      breakdown: [
        { name: "Encoding", value: costs.encoding },
        { name: "Storage", value: costs.storage },
        { name: "Delivery", value: costs.delivery },
        { name: "Other", value: costs.other },
      ],
    },
  ]

  const chartData = [{ name: "Revenue vs. Cost", Revenue: revenue.totalRevenue, Cost: totalCost }]

  const isProfit = revenue.netOperatingProfit >= 0
      ...
      <CardContent className="p-6 space-y-6">
          ...
          <div className="flex flex-col space-y-4">
            ...
            <div className="space-y-1">
              <div className="flex justify-between text-sm">
                <span>Revenue</span>
                <span className="font-mono">{formatCurrency(revenue.totalRevenue)}</span>
              </div>
              <div className="w-full bg-slate-100 dark:bg-slate-800 rounded-full h-4 overflow-hidden">
                <div
                  ...
                  style={{
                    width: `${Math.min(100, (revenue.totalRevenue / Math.max(revenue.totalRevenue, totalCost)) * 100)}%`,
                  }}
                />
              </div>
              <div className="flex justify-between text-xs text-slate-500 dark:text-slate-400">
                <span>Live Ads: {formatCurrency(revenue.liveAdRevenue)}</span>
                <span>Paid: {formatCurrency(revenue.paidProgrammingRevenue)}</span>
                <span>VOD: {formatCurrency(revenue.vodAdRevenue)}</span>
              </div>
            </div>
            ...
            <div className="space-y-1">
              ...
              <div className="w-full bg-slate-100 dark:bg-slate-800 rounded-full h-4 overflow-hidden">
                <div
                  ...
                  style={{
                    width: `${Math.min(100, (totalCost / Math.max(revenue.totalRevenue, totalCost)) * 100)}%`,
                  }}
                />
              </div>
              <div className="flex flex-wrap justify-between text-xs text-slate-500 dark:text-slate-400">
                <span>Encoding: {formatCurrency(costs.encoding)}</span>
                <span>Storage: {formatCurrency(costs.storage)}</span>
                <span>Delivery: {formatCurrency(costs.delivery)}</span>
                <span>Other: {formatCurrency(costs.other)}</span>
              </div>
            </div>
            ...
            <div className="mt-4 pt-4 border-t border-slate-200 dark:border-slate-700">
              ...
              <div className="mt-2 w-full bg-slate-100 dark:bg-slate-800 rounded-full h-2 overflow-hidden">
                <div
                  ...
                  style={{
                    width: `${Math.min(100, (Math.abs(revenue.netOperatingProfit) / Math.max(revenue.totalRevenue, totalCost)) * 100)}%`,
              ...
              <div className="mt-2 text-xs text-center text-slate-500 dark:text-slate-400">
                {isProfit
                  ? `${((revenue.netOperatingProfit / revenue.totalRevenue) * 100).toFixed(1)}% profit margin`
                  : `${((Math.abs(revenue.netOperatingProfit) / totalCost) * 100).toFixed(1)}% loss ratio`}
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="bg-slate-50 dark:bg-slate-900/50 p-4 rounded-lg">
          ...
          <div className="flex items-center justify-between">
            ...
            <motion.div
              key={revenue.netOperatingProfit}
          ...
          <div>
            ...
            <div className="space-y-2">
              ...
              <div className="pt-2 border-t border-slate-200 dark:border-slate-700 flex justify-between items-center">
                ...
                <span className="font-medium font-mono">{formatCurrency(revenue.totalRevenue)}</span>
              </div>
            </div>
          </div>
